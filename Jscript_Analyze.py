...
def dom_xss_scanner(regex_str,jscript_file):
    ...
    for items in Dom_Xss_Sinks:
        ...
        for key in tmp_item:
            if "Property_Susceptible" in key:
                ...
                try:
                    result = dom_xss_scanner(tmp_item["Property_Susceptible"],jscript_file)
                    if result:
                       print(result)
...
def main():
    ...
    for target_to_check in target_list:
        print(target_to_check.strip())

        try:
           js_paths = []
           results = javascript_grabber(target_to_check)#enter target it will give you urls of javascripts download put in dir
